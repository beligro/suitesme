...
func (ctr StylesController) List(ctx echo.Context) error {
	...
	styles := ctr.storage.Styles.List(sort, order, limit, offset)

	ctx.Response().Header().Set("X-Total-Count", fmt.Sprintf("%d", ctr.storage.Styles.Count()))
	ctx.Response().Header().Set("Access-Control-Expose-Headers", "X-Total-Count")

	return ctx.JSON(http.StatusOK, styles)
}
