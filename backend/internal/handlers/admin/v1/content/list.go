...
func (ctr ContentController) List(ctx echo.Context) error {
	...
	content := ctr.storage.WebContent.List(sort, order, limit, offset)

	ctx.Response().Header().Set("X-Total-Count", fmt.Sprintf("%d", ctr.storage.WebContent.Count()))
